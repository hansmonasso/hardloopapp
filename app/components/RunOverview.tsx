...
import { useSearchParams } from 'next/navigation' // NIEUW
...
export default function RunOverview({ runs, user }: RunOverviewProps) {
  ...
  const highlightId = searchParams.get('run_id') // Haal ID uit URL
  ...
  const filteredRuns = runs.filter(run => {
    // NIEUW: Als dit het 'gedeelde' loopje is, laat hem ALTIJD zien (negeer filters)
      ...
      {user ? (
        <>
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
             ...
             <Link href="/calculator" className="bg-white border border-gray-300 text-black px-6 py-3 rounded-full font-medium hover:bg-gray-50 transition flex items-center justify-center gap-2"><span>🧮</span> Calculator</Link>
          </div>
              ...
              <div className={isCompactView ? "flex flex-col gap-3" : "grid gap-6 md:grid-cols-2"}>
                {filteredRuns.map((run: any) => (
                  <RunCard 
                      ...
                      // NIEUW: Geef door of dit het speciale loopje is
