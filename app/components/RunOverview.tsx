...
export default function RunOverview({ runs, user }: RunOverviewProps) {
  ...
  const filteredRuns = runs.filter(run => {
    // Als dit het 'gedeelde' loopje is via de link, laat hem ALTIJD zien
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center mb-8 flex-wrap">
         ...
         <Link href="/about" className="bg-white border border-gray-300 text-black w-12 h-12 rounded-full font-medium hover:bg-gray-50 transition flex items-center justify-center" title="Hoe werkt het?">
            ℹ️
         </Link>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-200 dark:border-gray-700 mb-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          <div className="lg:col-span-1">
            <input type="text" placeholder="Zoek plaats..." value={filterLocation} onChange={(e) => setFilterLocation(e.target.value)} className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900" />
          </div>
          
          <div>
            <input type="date" value={filterStartDate} onChange={(e) => setFilterStartDate(e.target.value)} className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-500" />
          </div>
          
          <div>
            <input type="date" value={filterEndDate} onChange={(e) => setFilterEndDate(e.target.value)} className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900 text-gray-500" />
          </div>
          
          <div>
            <input type="number" placeholder="Min km" value={filterMinDist} onChange={(e) => setFilterMinDist(e.target.value)} className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900" />
          </div>
          
          <div>
            <input type="number" placeholder="Max km" value={filterMaxDist} onChange={(e) => setFilterMaxDist(e.target.value)} className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-900" />
          </div>
